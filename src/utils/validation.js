...
const ruleBuilder = (rules, value, data, pkey, index, parentValue) => {
  ...
  if (!Array.isArray(rules) && typeof rules === 'object') {
    return ruleBuilder([rules], value, data, pkey);
  }

  return rules.map((rule) => {
    if (Array.isArray(value)) {
      return value.map((valueChild, i) => {
        return ruleBuilder(rules, valueChild, data, pkey, i, value);
      });
    } else if (value && typeof value === 'object') {
      const error = {};
      Object.keys(value).forEach((key) => {
        error[key] = ruleBuilder(rule[key], value[key], data, key, index, value);
      });
      return error;
    }
    return rule(value, data, pkey, index, parentValue);
...
export function titleReg(value) {
  const validTitle = /^([m][s][(][.][)])?([m][r][(][.][)])?([m][r][s][(][.][)])|mark$|mary$/;
  if (typeof value !== 'undefined' && value.match(validTitle)) {
    return '不可為 “ms(.)”, “mr(.)”,  “mrs(.)” 開頭, 不可為 “mark” or “mary” 結尾';
  }
}
...
export function maxString(max) {
  return value => {
    if (typeof value !== 'undefined' && value.length > max) {
      return `Please enter no more than ${max} words.`;
    }
  };
}
...
export function createAsyncValidatorStatusName(key) {
  let isAsyncCheckedBefore = false;

  return (asyncValidating) => {
    ...
    if (!asyncValidating && isAsyncCheckedBefore) {
      isAsyncCheckedBefore = false;
      return 'success';
    }
    return '';
