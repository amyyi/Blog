...
const baseStorage = (() => {
  ...
  return {
    ...
    key: function(pindex = 0) { // 拿取array中的第幾個值
...
export default function createWebStorage() {
  ...
  return {
    ...
    set: function(pkey, pvalue, pexpire) {
      ...
      const expire = checkExpire(pexpire); // 到期日
      const updateTime = new Date().getTime(); // timestamp format
      const expireTime = (expire === 0) ? 0 : updateTime + expire * 1000; // 現在時間再加多少就是到期日，如果有更新就把到期日往上加
